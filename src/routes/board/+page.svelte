<script>
	...
	/**
	 * Prev/next walks the board in reading order: down each column, then on to
	 * the next. It's built from `columns`, so it honours the current search and
	 * stale/due filters exactly as shown.
	 */
	const orderedIds = $derived(ZONES.flatMap((zone) => (columns[zone] ?? []).map((p) => p.id)));
